...
如果想要实现滑动视图分页，可以为`ScrollViews`配置`pagingEnabled`属性。还可以使用组件[viewpagerandroid](viewpagerandroid)在Android上实现视图之间的水平滑动。

在iOS上 `ScrollView`允许用户缩放文本。你可以通过为`ScrollView`设置 `maximumZoomScale` 和 `minimumZoomScale`属性来禁止用户的缩放。
